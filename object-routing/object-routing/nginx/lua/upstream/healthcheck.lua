-- forked from https://github.com/openresty/lua-resty-upstream-healthcheck

-- (guojian): the original version only support upstream,
-- we modify it for dynamic upstream with balancers
...
local function set_peer_down_globally(ctx, is_backup, id, value)
    ...
    key = gen_peer_key("init:", u, is_backup, id)
    ok, err = dict:set(key, false)
    if not ok then
        errlog("failed to set peer init state: ", err)
    end
end
...
local function peer_fail(ctx, is_backup, id, peer)
    ...
    debug("peer ", peer.name, " fails ", fails)
    ...
    if (peer.init or not peer.down) and fails >= ctx.fall then
...
local function peer_ok(ctx, is_backup, id, peer)
    ...
    debug("peer ", peer.name, " succ ", succ)
    ...
    if (peer.init or peer.down) and succ >= ctx.rise then
...
local function check_peer(ctx, id, peer, is_backup)
    ...
    if peer.host then
        debug("peer host: ", peer.host, " port: ", peer.port)
    ...
    if concur <= 1 then
        ...
        if n <= concur then
            ...
            for i = 1, nthr do
                ...
                threads[i] = spawn(check_peer, ctx, peers[i].id, peers[i], is_backup)
            end
            ...
            check_peer(ctx, peers[n].id, peers[n], is_backup)
...
local function upgrade_peers_version(ctx, peers, is_backup)
    ...
    for i = 1, n do
        ...
        key = gen_peer_key("init:", u, is_backup, id)
        res, err = dict:get(key)
        if res == nil then -- should not use not res since the value is false
            if err then
                errlog("failed to get peer init state: ", err)
            end
        else
            peer.init = res
        end
...
local function do_check(ctx)
    ...
    local ppeers, err = ctx.get_primary_peers(ctx.upstream)
    if not ppeers then
        errlog("failed to get primary peers: ", err)
    end
    local bpeers, err = ctx.get_backup_peers(ctx.upstream)
    if not bpeers then
    ...
    if ctx.new_version then
        ...
        if ctx.notify then
            debug("notify with ctx version ", ctx.version, ", primary: ", cjson.encode(ctx.primary_peers), ", backup: ", cjson.encode(ctx.backup_peers))
            ctx.notify(ctx)
        end
    end
end
...
function _M.spawn_checker(opts)
    ...
    local get_primary_peers = opts.get_primary_peers or get_primary_peers
    local get_backup_peers = opts.get_backup_peers or get_backup_peers
    local set_peer_down = opts.set_peer_down or set_peer_down
    ...
    local ctx = {
        ...
        notify = notify, -- callback if peers changed (new version)
        http_req = http_req,
        timeout = timeout,
        interval = interval,
        dict = dict,
...
function _M.status_page(opts)
    ...
    local get_upstreams = opts.get_upstreams or get_upstreams
